...
public final class StringTexture extends Texture {
    ...
    public static int computeTextWidthForConfig(float textSize, Typeface typeface, String string) {
        ...
        synchronized (paint) {
        	if(string == null || string.length() == 0){
	            ...
	            // 4 pixel(before : 10 pixel) buffer to compensate for the shade at the end.
	            return (int) (4.0f) + (int) FloatMath.ceil(paint.measureText(string));
        	}
        }
    }
    ...
    public static int lengthToFit(float textSize, float maxWidth, Typeface typeface, String string) {
        ...
        // BEGIN: 0014682 sooyoul.ko@lge.com 2011-01-24
        // MOD: 0014682: [Gallery3D] modify function lengthToFit.
        if(length < 1)
        	return length;
        float retVal = paint.measureText(string, 0, length - 1);
        // END: 0014682 sooyoul.ko@lge.com 2011-01-24
        if (retVal <= maxWidth)
            return length;
        else {
    ...
    public static final class Config {
        ...
        public float b = 1f;
        public float a = 1f;
        public int shadowRadius = 4 * 1;//(int) App.PIXEL_DENSITY;
