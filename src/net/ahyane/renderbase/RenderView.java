...
public class RenderView extends GLSurfaceView implements GLSurfaceView.Renderer, SensorEventListener {
    ...
    private static final float HIT_AREA = 10.0f;
    ...
    protected RootLayer mRootLayer = null;
    ...
    public static final class Lists {
        ...
        public final ArrayList<Layer> animationList = new ArrayList<Layer>();
        ...
        try {
            ...
            if (bitmap != null) {
                ...
                if (!Shared.isPowerOf2(width) || !Shared.isPowerOf2(height)) {
                    ...
                    //패딩을 만들지 않고 비트맵크기를 조절함
//                    Bitmap padded = Bitmap.createBitmap(paddedWidth, paddedHeight, config);
//                    Canvas canvas = new Canvas(padded);
//                    canvas.drawBitmap(bitmap, 0, 0, null);
//                    bitmap.recycle();
//                    bitmap = padded;
                    Bitmap resizedBitmap = Bitmap.createScaledBitmap(bitmap, paddedWidth, paddedHeight, true);
                    bitmap.recycle();
    ...
    private void uploadTexture(Texture texture, int[] textureId) {
        ...
        if (bitmap != null) {
            ...
            gl.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, texture.mTextureWrap);
            gl.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T, texture.mTextureWrap);
    ...
    public void onDrawFrame(GL10 gl1) {
	    ...
	    while(true){
	        ...
	        frameTime += nowMillis - frameStartTime;
	        frameStartTime = nowMillis;
        ...
        synchronized (lists) {
            ...
            final ArrayList<Layer> animationList = lists.animationList;
            for (int i = animationList.size() - 1; i >= 0; --i) {
                final Layer layer = animationList.get(i);
                if (!layer.mHidden && layer.mLayerAnimation != null) {
                	if(layer.mLayerAnimation.isStarted()){
                		layer.mLayerAnimation.onPlay(this, layer);
                	}
            		//Log.e("ANI", "ANIMATION   " + layer.mLayerAnimation.isStarted());
                }
            }
            ...
            for (int i = opaqueList.size() - 1; i >= 0; --i) {
                ...
                if (!layer.mHidden) {
                	//if this object is 2d object, orthogonal. else perspective.
            		if(layer.is2D())RenderUtils.setOrtho(this, gl);
            		else RenderUtils.setPerspective(this, gl);
    ...
    private Layer hitTest(float x, float y) {
        ...
        for (int i = hitTestList.size() - 1; i >= 0; --i) {
            ...
            if (layer != null && !layer.mHidden) {
                ...
                if (x >= layerX - HIT_AREA - layer.mHitArea
                && y >= layerY - HIT_AREA - layer.mHitArea
                && x < layerX + layer.mWidth + HIT_AREA + layer.mHitArea
                && y < layerY + layer.mHeight + HIT_AREA + layer.mHitArea
